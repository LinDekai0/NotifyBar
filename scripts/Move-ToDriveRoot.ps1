...
if (!(Test-Path -LiteralPath (Join-Path $sourcePath 'NotificationBarrage.sln'))) { throw '源目录不含预期解决方案，已停止。' }
if ($sourcePath -ne $destinationPath -and (Test-Path -LiteralPath $destinationPath)) { throw 'D:\notification-barrage 已存在，不会覆盖或合并。' }
...
if ($sourcePath -ne $destinationPath) {
    ...
    if (!(Test-Path -LiteralPath (Join-Path $destinationPath 'NotificationBarrage.sln')) -or (Test-Path -LiteralPath $sourcePath)) { throw '移动结果异常，请检查两个目录，不要再次覆盖执行。' }
}
